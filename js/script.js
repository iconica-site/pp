...
const productCap = document.querySelector(".product__cap");
    ...
    switch (true) {
      case matchMedia("(min-width: 1440.1px)").matches:
        if (compoundBlock.getBoundingClientRect().top > innerHeight) {
          ...
          setProperty(productCap, "--cap-top", compoundBlock, -132.72, -372.64);
        }
      ...
      default:
        if (compoundBlock.getBoundingClientRect().top > innerHeight) {
          ...
          setProperty(productCap, "--cap-top", compoundBlock, -132.72, -540.85);
        }
    }
  });
}
